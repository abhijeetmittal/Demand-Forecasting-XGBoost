...
#5 Do random search and run model 50 times:
rancontrol <- makeTuneControlRandom(maxit = 5L)



#6 tune parameters:
tune57 <- tuneParams(learner = rf.lrn
                   ,task = traintask57,resampling = set_cv,measures = list(mae),par.set = params,control = rancontrol,show.info = T)
tune77 <- tuneParams(learner = rf.lrn
                     ,task = traintask77,resampling = set_cv,measures = list(mae),par.set = params,control = rancontrol,show.info = T)


rf.tree <- setHyperPars(rf.lrn, par.vals = tune77$x)
rforest <- train(rf.tree, traintask77)
getLearnerModel(t.rpart)
rfmodel <- predict(rforest, testtask)

rf57 <- randomForest(x = dtrain57, y = labels57, ntree = 341, mtry = 10, nodesize = 13)
rf77 <- randomForest(x = new_tr77, y = labels77, ntree =185 , mtry=8 , nodesixe =11 )

rfmodel <- predict(rf77, new_ts77)
test77 <- test77[,predictions:=rfmodel]


write.csv(test77, "test77_rf.csv", row.names = F)
